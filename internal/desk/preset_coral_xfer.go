...
func PresetCoralXfer(tenantID string) Doc {
	...
	d := Doc{
		...
		Tone:            "professional",
		VoiceID:         "ritu",
		Voice:           map[string]string{"sarvam-tts": "ritu"},
		CX: CXPolicy{
			BargeIn:                true,
...
func xferPrompts() map[string]Prompt {
	list := []Prompt{
		...
		xferPrompt("ask_language", "Ask preferred language",
			"Which language would you prefer? Hindi, English, or another Indian language."),
		xferPrompt("language_confirmed", "Language confirmed",
			"Understood."),
		xferPrompt(PromptClarify, "Department menu",
			"Sales, Corporate, or Support — which do you need?"),
		xferPrompt("clarify_2", "Clarify again",
			"Please say Sales, Corporate, or Support."),
